...
- Codex same-provider fork is lazy: `thread.fork` records lineage and pending transfer, and first
  dispatch resolves the native Codex fork. Earlier source-point forks pass the boundary turn through
  `thread/fork.lastTurnId`, which also works with paginated Codex histories.
